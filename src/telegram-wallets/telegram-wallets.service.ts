...
export class TelegramWalletsService {
    ...
    async getListBuyTokens(user: any) {
        try {
            ...
            const BITT_MINT_DEVNET = '9wcpBxUDTi2K7cXoHsmP7K4S7ZSZpjedQrR3gh1evVNQ';
            let bittBalance = 0;
            if (missingDefaultTokens.includes(BITT_MINT_DEVNET)) {
                try {
            ...
            if (missingDefaultTokens.includes(USDT_MINT_DEVNET)) {
                ...
                const usdtAccount = tokenAccounts.find(account => account.mint === USDT_MINT_DEVNET);
                if (usdtAccount) {
            ...
            if (missingDefaultTokens.includes(BITT_MINT_DEVNET)) {
                allTokenAccounts.push({
                    mint: BITT_MINT_DEVNET,
                    amount: bittBalance
                });
            } else {
                // Find BITT in user's token accounts and add it third
                const bittAccount = tokenAccounts.find(account => account.mint === BITT_MINT_DEVNET);
                if (bittAccount) {
            ...
            const otherTokens = tokenAccounts.filter(account => 
                ...
                account.mint !== USDT_MINT_DEVNET &&
                account.mint !== BITT_MINT_DEVNET
            );
            ...
            // SolanaTracker removed - using database data only
            ...
            const tokens = await Promise.all(allTokenAccounts.map(async (account) => {
                ...
                if (account.mint === USDT_MINT_DEVNET) {
                    // Lấy giá USDT thực tế nếu có
                    const tokenPrice = await this.solanaService.getTokenPricesInRealTime([USDT_MINT_DEVNET]);
                    const priceUSD = tokenPrice?.get(USDT_MINT_DEVNET)?.priceUSD || 1;
                    const priceSOL = tokenPrice?.get(USDT_MINT_DEVNET)?.priceSOL || 0;
                    return {
                        token_address: USDT_MINT_DEVNET,
                        token_name: 'USDT',
                        token_symbol: 'USDT',
                ...
                if (account.mint === BITT_MINT_DEVNET) {
                    // Lấy giá BITT thực tế nếu có
                    const tokenPrice = await this.solanaService.getTokenPricesInRealTime([BITT_MINT_DEVNET]);
                    const priceUSD = tokenPrice?.get(BITT_MINT_DEVNET)?.priceUSD || 0;
                    const priceSOL = tokenPrice?.get(BITT_MINT_DEVNET)?.priceSOL || 0;
                    return {
                        token_address: BITT_MINT_DEVNET,
                        token_name: 'BITT',
                        token_symbol: 'BITT',
