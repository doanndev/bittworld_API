                            ...
                            Object.entries(response?.data?.tokens).map(async ([address, tokenData]: [string, any]) => {
                                ...
                                if (tokenData.token?.uri) {
                                    try {
                                        ...
                                        // Thử fallback với proxy nếu lỗi 403
                                        if (error.response?.status === 403) {
                                            try {
                                                this.logger.warn(`403 error for ${address}, trying proxy fallback`);
                                                const proxyUrl = `https://image.solanatracker.io/proxy?url=${encodeURIComponent(tokenData.token.uri)}`;
                                                const { data: proxyData } = await axios.get(proxyUrl, { 
                                                    timeout: 3000,
                                                    headers: {
                                                        'Accept': 'application/json',
                                                ...
                                                logo_uri = proxyData?.image || null;
                                            } catch (proxyError) {
                                                this.logger.warn(`Proxy fallback also failed for ${address}: ${proxyError.message}`);
                                            }
                                        } else {
