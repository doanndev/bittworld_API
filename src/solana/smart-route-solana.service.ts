...
export class SmartRouteSolanaService implements OnModuleInit {
    ...
    private maxRetries = 3; // Reduced retries for devnet
    ...
    async onModuleInit() {
        try {
            ...
            // Khởi tạo Jupiter trong background (non-blocking)
            this.jupiterInitPromise = this.initJupiterWithRetry();
            this.jupiterInitPromise.catch(error => {
                // Error already handled in initJupiterWithRetry, just log here
                console.warn('Jupiter initialization completed with warnings (non-critical)');
            });
        } catch (error) {
    ...
    async initJupiterWithRetry(attempt = 1) {
        try {
            ...
            // Only log detailed error on first attempt, then just warn
            if (attempt === 1) {
                console.error(`Jupiter initialization failed (attempt ${attempt}):`, error.message || error);
            } else {
                console.warn(`Jupiter initialization failed (attempt ${attempt}/${this.maxRetries}): ${error.message || 'Network error'}`);
            }
            ...
            if (attempt < this.maxRetries) {
                ...
                // Don't throw error, just log warning and continue without Jupiter
                this.jupiter = null;
                this.isJupiterReady = false;
                console.warn('⚠️  Jupiter API initialization failed after all retries. Swap functionality may be limited.');
                console.warn('   This is expected in devnet if Jupiter API is unavailable.');
                // Don't throw - allow app to continue without Jupiter
            }
        }
    }
