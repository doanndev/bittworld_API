...
export class SolanaWebSocketService implements OnModuleInit, OnModuleDestroy {
    ...
    private disconnect() {
        if (this.ws) {
            try {
                // Check WebSocket state before terminating
                // 0 = CONNECTING, 1 = OPEN, 2 = CLOSING, 3 = CLOSED
                if (this.ws.readyState === 0) {
                    // WebSocket is still connecting, just close it normally
                    this.ws.close();
                } else if (this.ws.readyState === 1) {
                    // WebSocket is open, terminate it
                    this.ws.terminate();
                } else {
                    // WebSocket is already closing or closed, just mark as disconnected
                    this.logger.debug('WebSocket already closing/closed');
                }
            } catch (error) {
    ...
    async trackTransactions(accountAddress: PublicKey) {
        ...
        if (this.ws && this.ws.readyState === 1) {
            ...
            this.logger.warn('WebSocket not ready, cannot track transactions');
        }
    }
    ...
    async trackTransactionStatus(signature: string) {
        ...
        if (this.ws && this.ws.readyState === 1) {
            ...
            this.logger.warn('WebSocket not ready, cannot track transaction status');
        }
    }
    ...
    async trackAccountBalance(accountAddress: PublicKey) {
        ...
        try {
            // Check if WebSocket is available before attempting to subscribe
            if (!this.isConnected || !this.ws || this.ws.readyState !== 1) {
                this.logger.warn(`WebSocket not ready for account ${accountKey}, skipping subscription`);
            ...
            // Don't throw error, just log and emit fallback event
            this.logger.warn(`Failed to track balance for account ${accountKey}: ${error.message}`);
            // Remove from subscriptions if it was added
            this.subscriptions.delete(`balance:${accountKey}`);
            // Fallback to RPC if WebSocket fails
            this.eventEmitter.emit('websocket.fallback', {
                type: 'accountBalance',
                account: accountKey
    ...
    async trackAccountChanges(accountAddress: PublicKey) {
        ...
        if (this.ws && this.ws.readyState === 1) {
            ...
            this.logger.warn('WebSocket not ready, cannot track account changes');
        }
    }
    ...
    async unsubscribe(key: string) {
        ...
        try {
            ...
            this.logger.warn(`Failed to send unsubscribe message: ${error.message}`);
        }
        ...
        try {
            ...
            if (!this.isHealthy) {
                ...
                try {
                    ...
                    this.logger.warn(`Error during disconnect in health check: ${disconnectError.message}`);
                }
                try {
                    await this.connect();
                } catch (connectError) {
                    this.logger.error(`Failed to reconnect in health check: ${connectError.message}`);
            ...
            // Try to reconnect
            try {
                // Safely disconnect without throwing
                try {
                    this.disconnect();
                } catch (disconnectError) {
                    // Ignore disconnect errors
    ...
    private async sendRequest(method: string, params: any[] = []): Promise<any> {
        ...
        if (!this.isHealthy) {
            try {
                ...
                // If health check fails, don't throw immediately
                this.logger.warn(`Health check failed: ${healthError.message}`);
            }
        ...
        try {
            ...
            this.logger.warn(`Failed to ensure connection: ${connectError.message}`);
