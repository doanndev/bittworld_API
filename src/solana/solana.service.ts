...
export class SolanaService {
    ...
    async getBalance(address: string): Promise<number> {
        try {
            ...
            // Bắt đầu theo dõi thay đổi (non-blocking, don't fail if WebSocket has issues)
            try {
                await this.solanaWebSocketService.trackAccountBalance(publicKey);
            } catch (wsError) {
                // Log warning but don't fail the balance request
                this.logger.warn(`Failed to track balance via WebSocket for ${address}: ${wsError.message}`);
            }
    ...
    private async getTokenPriceFromJupiter(tokenAddress: string): Promise<TokenPriceResponseDto> {
        try {
            // Devnet token pricing logic
            if (tokenAddress === this.USDT_MINT_DEVNET) {
                // USDT devnet: Get real USDT price from market, fallback to 1 USD
                try {
                    const realUsdtMint = 'Es9vMFrzaCERmJfrF4H2FYD4KCoNkY11McCe8BenwNYB'; // Mainnet USDT
                    const response = await axios.get(`https://api.jup.ag/price/v2?ids=${realUsdtMint},${this.SOL_MINT}`);
                    const usdtData = response.data.data[realUsdtMint];
                    const solData = response.data.data[this.SOL_MINT];
                    
                    if (usdtData?.price && solData?.price) {
                        const usdtPrice = parseFloat(usdtData.price);
                        const solPrice = parseFloat(solData.price);
                        return {
                            priceUSD: usdtPrice,
                            priceSOL: solPrice > 0 ? usdtPrice / solPrice : 0
                        };
                    }
                } catch (error) {
                ...
                // Fallback: USDT = 1 USD
                // Try to get SOL price, but if it fails, use default SOL price
                let solPrice = 150; // Default SOL price fallback (~$150)
                try {
                    const solPriceResponse = await axios.get(`https://api.jup.ag/price/v2?ids=${this.SOL_MINT}`, {
                        timeout: 5000 // 5 second timeout
                    });
                    const solData = solPriceResponse.data.data[this.SOL_MINT];
                    if (solData?.price) {
                        solPrice = parseFloat(solData.price);
                    }
                } catch (error) {
                    this.logger.warn(`Failed to get SOL price for USDT fallback, using default: ${error.message}`);
                }
                return {
                    priceUSD: 1,
                    priceSOL: solPrice > 0 ? 1 / solPrice : 0
                };
            }

            if (tokenAddress === this.BITT_MINT_DEVNET) {
                // BITT devnet: Always equals USDT price (1 BITT = 1 USDT)
                const usdtPrice = await this.getTokenPriceFromJupiter(this.USDT_MINT_DEVNET);
                return usdtPrice;
            }

            // For other tokens (including SOL), use Jupiter API
            if (tokenAddress === this.SOL_MINT) {
                ...
                try {
                    ...
                    this.logger.warn(`Failed to get SOL price from Jupiter (${error.response?.status || error.message}), using fallback`);
                    // Fallback SOL price
                    return {
                        priceUSD: 150, // Default SOL price fallback
                        priceSOL: 1
                    };
                }
            }
            ...
            try {
                ...
                this.logger.warn(`Jupiter API failed for ${tokenAddress} (${error.response?.status || error.message})`);
                return { priceUSD: 0, priceSOL: 0, error: error.message };
            }
        } catch (error) {
            this.logger.error(`Error getting token price from Jupiter: ${error.message}`);
            // For SOL, return fallback price
            if (tokenAddress === this.SOL_MINT) {
                return { priceUSD: 150, priceSOL: 1, error: 'Using fallback price' };
            }
            return { priceUSD: 0, priceSOL: 0, error: error.message };
        }
    }
        ...
        try {
            ...
            for (const address of missingAddresses) {
                try {
                    ...
                    this.logger.warn(`Failed to get price for ${address} from Jupiter: ${error.message}`);
                    prices.set(address, { priceUSD: 0, priceSOL: 0 });
                }
            }
    ...
    async getTokenPriceInRealTime(tokenAddress: string): Promise<TokenPriceResponseDto> {
        try {
            ...
            return {
                priceUSD: 0,
                priceSOL: 0,
                error: 'Failed to get price from Jupiter'
            };
        } catch (error) {
