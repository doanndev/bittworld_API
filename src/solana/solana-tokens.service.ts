...
export class SolanaTokensService {
    ...
    constructor(
        ...
        // private chatsService: ChatsService, // Removed - chat module disabled
    ...
    async findByAddress(address: string): Promise<{ status: number; data?: SolanaTokenDto; message?: string }> {
        try {
            ...
            try {
                ...
                // SolanaTracker removed - using basic token info only
                const tokenDataSolanaTracker = null;
                // try {
    ...
    async getMyWishlist(walletId: number): Promise<{ status: number; data?: any; message?: string }> {
        try {
            ...
            const multiTokensData: { data: Array<{ address?: string; name?: string; symbol?: string; logo_uri?: string; holders?: number; volume_1h_usd?: number; volume_24h_usd?: number; volume_4h_change_percent?: number; volume_1h_change_percent?: number; volume_24h_change_percent?: number; market_cap?: number; liquidity?: number; price?: number; program?: string }> } = { data: [] };
    ...
    async getTokenInfo(tokenAddress: string): Promise<SolanaTokenDto> {
        try {
            ...
            // SolanaTracker removed - token must exist in database
