...
export class BittPriceGateway implements OnGatewayConnection, OnGatewayDisconnect, OnGatewayInit {
  @WebSocketServer()
  server: Server;
  ...
  constructor() {
    // Tránh kết nối tại constructor vì @WebSocketServer có thể chưa sẵn sàng
  }
  ...
  private handleBittworldMessage(message: any) {
    if (message.method === 'price.update' && message.params && message.params.length >= 2) {
      ...
      if (!this.server) {
        this.logger.warn('Socket server chưa sẵn sàng, cache giá và bỏ qua emit.');
        return;
      }
