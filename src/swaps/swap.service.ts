...
export class SwapService {
  ...
  private readonly MAX_RETRY_ATTEMPTS = 3; // Số lần thử tối đa cho việc chuyển SOL hỗ trợ
  ...
  /**
   * Chuyển SOL hỗ trợ phí từ ví hỗ trợ đến ví user
   */
    ...
    try {
      ...
      switch (createSwapDto.swap_type) {
        case SwapOrderType.USDT_TO_SOL:
          ...
          // USDT sang SOL cũng cần kiểm tra SOL balance để trả phí giao dịch
          if (hasBalance) {
            const solBalanceForFee = await this.connection.getBalance(userKeypair.publicKey);
            const minSolForTransaction = 0.00001; // ~0.00001 SOL cho phí transaction
            const balanceInSolForFee = solBalanceForFee / 1e9; // Convert lamports to SOL
            
            if (balanceInSolForFee < minSolForTransaction) {
              this.logger.log(`USDT to SOL swap: User has sufficient USDT but insufficient SOL for transaction fee. Current: ${balanceInSolForFee} SOL, Required: ${minSolForTransaction} SOL`);
              // Vẫn cho phép tiếp tục vì sẽ được hỗ trợ phí ở bước 8.5
              hasBalance = true;
            }
          }
          break;
      ...
      // 8.5. Kiểm tra SOL balance và hỗ trợ phí mạng lưới nếu cần
      let solBalance = await this.connection.getBalance(userKeypair.publicKey);
      const minSolBalance = 0.001; // Tối thiểu 0.001 SOL
      const transferAmount = 0.001; // Chuyển 0.001 SOL

      if (solBalance < minSolBalance * LAMPORTS_PER_SOL) {
        this.logger.log(`Insufficient SOL balance (${solBalance / LAMPORTS_PER_SOL} SOL), need to transfer ${transferAmount} SOL to wallet ${wallet.wallet_solana_address}`);
        
        const supportFeePrivateKey = this.configService.get<string>('WALLET_SUP_FREE_SWAP');
        if (!supportFeePrivateKey) {
          throw new InternalServerErrorException('WALLET_SUP_FREE_SWAP configuration does not exist');
        }
        ...
        for (let solAttempt = 1; solAttempt <= this.MAX_RETRY_ATTEMPTS; solAttempt++) {
          try {
            this.logger.log(`Executing SOL fee transfer attempt ${solAttempt}/${this.MAX_RETRY_ATTEMPTS} for wallet ${wallet.wallet_solana_address}`);
            ...
            solBalance = await this.connection.getBalance(userKeypair.publicKey);
            
            if (solBalance < minSolBalance * LAMPORTS_PER_SOL) {
              throw new Error(`SOL balance still insufficient after fee transfer. Current: ${solBalance / LAMPORTS_PER_SOL} SOL`);
            }
            ...
            this.logger.error(`SOL fee transfer attempt ${solAttempt}/${this.MAX_RETRY_ATTEMPTS} failed: ${error.message}`);
