...
import { ConflictException, HttpException, HttpStatus, UnauthorizedException, BadRequestException } from '@nestjs/common';
...
import { StakeInfoResponseDto } from './dto/stake-info-response.dto';
...
export class AdminController {
  ...
  @UseGuards(JwtAuthAdminGuard)
  @Get('stake-history')
  @ApiOperation({ summary: 'Get stake history by UID, Wallet ID, or wallet address. If no search provided, returns all stake history (Admin only)' })
  @ApiQuery({ 
    name: 'search', 
    required: false, 
    description: 'UID, Wallet ID (number), or wallet address (Solana/ETH). If not provided, returns all stake history',
    example: '1234567890'
  })
  ...
  @ApiResponse({ 
    status: 200, 
    description: 'Stake history retrieved successfully',
    type: StakeInfoResponseDto
  })
  @ApiResponse({ 
    status: 404, 
    description: 'Wallet not found with the provided search parameter' 
  })
  @ApiResponse({ 
    status: 401, 
    description: 'Unauthorized - Only admin can view stake history' 
  })
  async getStakeHistory(
    ...
    return this.adminService.getStakeInfo(search, page, limit);
  }
}
