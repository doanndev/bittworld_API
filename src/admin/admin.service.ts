...
export class AdminService implements OnModuleInit {
  ...
  async getOrderHistory(page?: number, limit?: number, search?: string, status?: string, isBittworld?: string) {
    // Trading module removed - return empty data
    return {
      data: [],
      total: 0,
      page: page || 1,
  ...
  async getOrderStats(isBittworld?: string) {
    // Trading module removed - return empty stats
    return {
      total: 0,
      executed: 0,
  ...
  /**
   * Lấy danh sách tất cả BG affiliate trees (tối ưu hóa hiệu suất)
   */
  async getAllBgAffiliateTrees(currentUser?: UserAdmin, isBittworld?: 'true' | 'false'): Promise<any[]> {
    ...
    // Lấy tất cả root wallet IDs để batch query
    const rootWalletIds = trees.map(tree => tree.bat_root_wallet_id);
    
    // Batch query để lấy tất cả root wallets cùng lúc
    const rootWallets = await this.listWalletRepository.find({
      where: { wallet_id: In(rootWalletIds) },
      select: ['wallet_id', 'wallet_solana_address', 'wallet_nick_name', 'wallet_eth_address', 'isBittworld', 'bittworld_uid']
    });
    
    // Tạo map để lookup nhanh
    const rootWalletMap = new Map(rootWallets.map(wallet => [wallet.wallet_id, wallet]));
    
    // Format dữ liệu để trả về với thông tin wallet (không xây dựng cấu trúc cây phức tạp)
    let treesWithWalletInfo = trees.map(tree => {
      const rootWallet = rootWalletMap.get(tree.bat_root_wallet_id);
      
      // Tìm root node để lấy status
      const rootNode = tree.nodes?.find(node => node.ban_wallet_id === tree.bat_root_wallet_id);
      
      // Đếm total members đơn giản (chỉ đếm các thành viên con, không bao gồm root)
      const totalMembers = tree.nodes?.filter(node => node.ban_parent_wallet_id !== null).length || 0;
  ...
  private async getRootBgTreeStructure(rootWalletId: number, tree: any, allNodes: any[]): Promise<any> {
    ...
    const treeStructure = await this.buildHierarchicalTreeOptimized(rootWalletId, allNodes);
  ...
  private async getMemberTreeStructure(memberWalletId: number, bgAffiliateInfo: any, tree: any, allNodes: any[]): Promise<any> {
    ...
    /**
   * Xây dựng cấu trúc cây phân theo nhánh (tối ưu hóa)
   */
    private async buildHierarchicalTreeOptimized(parentWalletId: number, allNodes: any[]): Promise<any[]> {
      // Lấy tất cả wallet IDs cần thiết để batch query
      const allWalletIds = allNodes.map(node => node.ban_wallet_id);
      
      // Batch query để lấy tất cả wallets cùng lúc
      const wallets = await this.listWalletRepository.find({
        where: { wallet_id: In(allWalletIds) },
        select: ['wallet_id', 'wallet_solana_address', 'wallet_nick_name', 'wallet_eth_address']
      });
      
      // Tạo map để lookup nhanh
      const walletMap = new Map(wallets.map(wallet => [wallet.wallet_id, wallet]));
  
      // Batch query để lấy tất cả node stats cùng lúc
      const nodeStatsMap = await this.batchGetNodeStats(allWalletIds);
  
      return this.buildHierarchicalTreeRecursive(parentWalletId, allNodes, walletMap, nodeStatsMap);
    }
  
    /**
     * Xây dựng cấu trúc cây phân theo nhánh (recursive helper)
     */
    private buildHierarchicalTreeRecursive(
      parentWalletId: number, 
      allNodes: any[], 
      walletMap: Map<number, any>, 
      nodeStatsMap: Map<number, any>
    ): any[] {
      ...
      for (const child of children) {
        const wallet = walletMap.get(child.ban_wallet_id);
        const nodeStats = nodeStatsMap.get(child.ban_wallet_id) || { totalVolume: 0, totalTrans: 0 };
        ...
        const childNode = {
          ...
          children: this.buildHierarchicalTreeRecursive(child.ban_wallet_id, allNodes, walletMap, nodeStatsMap)
        };
    ...
    /**
     * Batch lấy node stats cho nhiều wallet IDs
     */
    private async batchGetNodeStats(walletIds: number[]): Promise<Map<number, any>> {
      if (walletIds.length === 0) {
        return new Map();
      }
  
      // Batch query để lấy trading volume từ trading_orders (giống getNodeStats gốc)
      const tradingStats = await this.dataSource.createQueryBuilder()
        .select('orders.order_wallet_id', 'walletId')
        ...
        .where('orders.order_wallet_id IN (:...walletIds)', { walletIds })
        .andWhere('orders.order_status = :status', { status: 'executed' })
        .groupBy('orders.order_wallet_id')
        .getRawMany();
  
      // Batch query để lấy master trading volume từ master_transaction_detail
      const masterTradingStats = await this.dataSource.createQueryBuilder()
        .select('mtd.mt_wallet_member', 'walletId')
        .addSelect('COALESCE(SUM(mtd.mt_detail_total_usd), 0)', 'totalVolume')
        .addSelect('COUNT(mtd.mt_detail_id)', 'totalTrans')
        .from('master_transaction_detail', 'mtd')
        .where('mtd.mt_wallet_member IN (:...walletIds)', { walletIds })
        .andWhere('mtd.mt_detail_status = :status', { status: 'success' })
        .groupBy('mtd.mt_wallet_member')
        .getRawMany();
  
      // Tạo map kết quả
      const statsMap = new Map<number, any>();
      
      // Khởi tạo tất cả wallet IDs với giá trị mặc định
      walletIds.forEach(walletId => {
        statsMap.set(walletId, { totalVolume: 0, totalTrans: 0 });
      });
  
      // Cập nhật trading stats
      tradingStats.forEach(stat => {
        const existing = statsMap.get(stat.walletId) || { totalVolume: 0, totalTrans: 0 };
        statsMap.set(stat.walletId, {
          totalVolume: parseFloat(stat.totalVolume) + existing.totalVolume,
          totalTrans: parseInt(stat.totalTrans) + existing.totalTrans
        });
      });
  
      // Cập nhật master trading stats
      masterTradingStats.forEach(stat => {
        const existing = statsMap.get(stat.walletId) || { totalVolume: 0, totalTrans: 0 };
        statsMap.set(stat.walletId, {
          totalVolume: parseFloat(stat.totalVolume) + existing.totalVolume,
          totalTrans: parseInt(stat.totalTrans) + existing.totalTrans
   ...
   */
    async getDashboardStatistics(isBittworld?: string): Promise<{
      ...
      // ==================== ORDER STATISTICS ====================
      // Trading module removed - return 0 for all order stats
      const totalOrders = 0;
      ...
      // Trading module removed - return 0 for all trading stats
      const tradingVolume = 0;
      const masterTradingVolume = 0;
      const totalVolume = 0;
      const averageOrderValue = 0;
      const mostActiveWallet = null;
      ...
      return {
        ...
        orders: {
          ...
          mostActiveWallet: null // Trading module removed
        },
        referrals: {
          traditionalReferrals: {
  ...
  async changeBgAffiliateFlow(data: {
    ...
    let targetBgInfo = await this.bgRefService.getWalletBgAffiliateInfo(data.walletId);
    
    // Lấy thông tin parent trước để có treeId
    ...
    // Nếu ví chưa có trong BG system thì tự động thêm vào
    if (!targetBgInfo) {
      // Tạo node BG mới cho ví này với treeId của parent
      const newBgNode = this.bgRefService['bgAffiliateNodeRepository'].create({
        ban_wallet_id: data.walletId,
        ban_parent_wallet_id: data.newParentWalletId,
        ban_tree_id: newParentBgInfo.treeId,
        ban_commission_percent: newParentBgInfo.commissionPercent,
        ban_status: true
      });
      await this.bgRefService['bgAffiliateNodeRepository'].save(newBgNode);
      
      // Lấy lại thông tin sau khi tạo
      targetBgInfo = await this.bgRefService.getWalletBgAffiliateInfo(data.walletId);
    }
    ...
    if (targetBgInfo && targetBgInfo.parentWalletId === null) {
    ...
    const oldParentWalletId = currentNode ? currentNode.ban_parent_wallet_id : null;
    const oldTreeId = targetBgInfo ? targetBgInfo.treeId : null;
    ...
    try {
      // 1. Cập nhật node cho ví cần di chuyển
      // Lấy lại node hiện tại (có thể đã được tạo ở trên)
      const currentNode = await this.bgRefService['bgAffiliateNodeRepository'].findOne({
        where: { ban_wallet_id: data.walletId }
      });
      ...
      // 3. Cập nhật commission percent nếu cần (chỉ áp dụng cho ví đã có trong BG system)
      if (currentNode && targetBgInfo && targetBgInfo.commissionPercent !== newParentBgInfo.commissionPercent) {
      ...
      return {
        ...
        nodeInfo: {
          ...
          oldTreeId,
          newTreeId: newParentBgInfo.treeId,
          newCommissionPercent: currentNode ? currentNode.ban_commission_percent : newParentBgInfo.commissionPercent,
  ...
  /**
   * Lấy thông tin stake của UID hoặc ví, hoặc tất cả nếu không có identifier
   * @param identifier UID, wallet_id hoặc địa chỉ ví (optional)
   * @param page Trang hiện tại
   * @param limit Số lượng records per page
   * @returns Thông tin stake của người dùng hoặc tất cả
   */
  async getStakeInfo(
    identifier?: string,
    page: number = 1,
    ...
    try {
      this.logger.log(`Getting stake info for identifier: ${identifier || 'ALL'}`);

      let queryBuilder = this.airdropPoolJoinRepository
        .createQueryBuilder('join')
        .leftJoinAndSelect('join.pool', 'pool')
        .leftJoinAndSelect('pool.originator', 'originator')
        .leftJoinAndSelect('join.member', 'member')
        .orderBy('join.apj_stake_date', 'DESC');

      let searchInfo: any = {
        type: 'all',
        value: 'ALL',
        wallet_id: null,
        wallet_solana_address: null,
        wallet_eth_address: null,
        wallet_nick_name: null,
        bittworld_uid: null,
        isBittworld: null
      };

      // Nếu có identifier, tìm ví cụ thể
      if (identifier) {
        let wallet: ListWallet | null = null;
        let searchType: 'uid' | 'wallet_id' | 'address' = 'address';

        // Thử tìm theo wallet_id (số)
        if (!isNaN(Number(identifier))) {
          wallet = await this.listWalletRepository.findOne({
            where: { wallet_id: Number(identifier) },
            relations: ['airdropPoolJoins', 'airdropPoolJoins.pool', 'airdropPoolJoins.pool.originator']
          });
          if (wallet) {
            searchType = 'wallet_id';
          }
        }

        // Nếu không tìm thấy, thử tìm theo bittworld_uid
        if (!wallet) {
          wallet = await this.listWalletRepository.findOne({
            where: { bittworld_uid: identifier },
            relations: ['airdropPoolJoins', 'airdropPoolJoins.pool', 'airdropPoolJoins.pool.originator']
          });
          if (wallet) {
            searchType = 'uid';
        ...
        // Nếu vẫn không tìm thấy, thử tìm theo địa chỉ Solana hoặc ETH
        if (!wallet) {
          wallet = await this.listWalletRepository.findOne({
            where: [
              { wallet_solana_address: identifier },
              { wallet_eth_address: identifier }
            ],
            relations: ['airdropPoolJoins', 'airdropPoolJoins.pool', 'airdropPoolJoins.pool.originator']
          });
          searchType = 'address';
        ...
        if (!wallet) {
          throw new NotFoundException('Wallet not found with the provided search parameter');
        }

        // Lọc theo wallet cụ thể
        queryBuilder = queryBuilder.where('join.apj_member = :walletId', { walletId: wallet.wallet_id });
      ...
      // Đếm tổng số records
      const totalStakes = await queryBuilder.getCount();
      ...
      const stakes = await queryBuilder
        .skip(offset)
        .take(limit)
        .getMany();

      // Tính tổng volume stake
      let totalVolumeQuery = this.airdropPoolJoinRepository
        .createQueryBuilder('join')
        .select('SUM(join.apj_volume)', 'totalVolume');

      if (identifier && searchInfo.wallet_id) {
        totalVolumeQuery = totalVolumeQuery.where('join.apj_member = :walletId', { walletId: searchInfo.wallet_id });
      }

      const totalVolumeResult = await totalVolumeQuery.getRawOne();
      const totalVolume = parseFloat(totalVolumeResult?.totalVolume || '0');

      // Đếm số pools đã tham gia
      let totalPoolsQuery = this.airdropPoolJoinRepository
        .createQueryBuilder('join')
        .select('COUNT(DISTINCT join.apj_pool_id)', 'totalPools');

      if (identifier && searchInfo.wallet_id) {
        totalPoolsQuery = totalPoolsQuery.where('join.apj_member = :walletId', { walletId: searchInfo.wallet_id });
      }

      const totalPoolsResult = await totalPoolsQuery.getRawOne();
      const totalPools = parseInt(totalPoolsResult?.totalPools || '0');

      // Transform dữ liệu
      const transformedStakes = stakes.map(stake => ({
        apj_id: stake.apj_id,
        apj_pool_id: stake.apj_pool_id,
        apj_member: stake.apj_member,
        apj_volume: parseFloat(stake.apj_volume.toString()),
        apj_stake_date: stake.apj_stake_date,
        apj_stake_end: stake.apj_stake_end,
        apj_round_end: stake.apj_round_end,
        apj_status: stake.apj_status,
        apj_hash: stake.apj_hash,
        pool: {
          alp_id: stake.pool.alp_id,
          alp_name: stake.pool.alp_name,
          alp_slug: stake.pool.alp_slug,
          alp_describe: stake.pool.alp_describe,
          alp_logo: stake.pool.alp_logo,
          alp_member_num: stake.pool.alp_member_num,
          apl_volume: parseFloat(stake.pool.apl_volume.toString()),
          apl_creation_date: stake.pool.apl_creation_date,
          apl_end_date: stake.pool.apl_end_date,
          apl_round_end: stake.pool.apl_round_end,
          apl_status: stake.pool.apl_status,
          alp_originator: stake.pool.alp_originator,
          originator_info: {
            wallet_id: stake.pool.originator.wallet_id,
            wallet_solana_address: stake.pool.originator.wallet_solana_address,
            wallet_eth_address: stake.pool.originator.wallet_eth_address,
            wallet_nick_name: stake.pool.originator.wallet_nick_name,
            bittworld_uid: stake.pool.originator.bittworld_uid
          }
        },
        member: {
          wallet_id: stake.member.wallet_id,
          wallet_solana_address: stake.member.wallet_solana_address,
          wallet_eth_address: stake.member.wallet_eth_address,
          wallet_nick_name: stake.member.wallet_nick_name,
          bittworld_uid: stake.member.bittworld_uid,
          isBittworld: stake.member.isBittworld
        }
      }));
      ...
      this.logger.error(`Error getting stake info: ${error.message}`);
      if (error instanceof NotFoundException) {
        throw error;
      }
      throw new BadRequestException(`Failed to get stake info: ${error.message}`);
    }
  }
}
