...
export class AirdropsService {
    ...
    private readonly MIN_REWARD_AMOUNT = 1000; // Minimum amount to get reward code (500,000 BITT)
    ...
    async createPool(walletId: number, createPoolDto: CreatePoolDto, logoFile?: Express.Multer.File) {
        ...
        return await this.redisLockService.withLock(lockKey, async () => {
            ...
            if (createPoolDto.initialAmount < this.MIN_REWARD_AMOUNT) {
                throw new BadRequestException('Initial amount must be at least 500,000');
            }
    ...
    async stakePool(walletId: number, stakePoolDto: StakePoolDto) {
        ...
        return await this.redisLockService.withLock(lockKey, async () => {
            ...
            // 3. Validate minimum stake amount based on user role and participation history
            ...
            // Check minimum stake amount for first-time participants (except pool creator)
            const MIN_FIRST_TIME_STAKE = 500000; // 500,000 BITT minimum for first participation
            if (!existingJoin && !isCreator && stakePoolDto.stakeAmount < MIN_FIRST_TIME_STAKE) {
                throw new BadRequestException('First-time participants must stake at least 500,000 BITT');
            }
            ...
            // Compare token balance (in token units) with stake amount (in token units)
            // getTokenBalance returns token units (already divided by decimals)
            ...
            // Calculate required raw units for transfer (transferTokenToBittWallet needs raw units)
